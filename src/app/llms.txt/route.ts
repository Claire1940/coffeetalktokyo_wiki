...
export async function GET() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://coffeetalktokyo.wiki'
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Coffee Talk Tokyo Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} provides Coffee Talk Tokyo recipes, character arcs, ending routes, and platform guides.`)
  lines.push('')
